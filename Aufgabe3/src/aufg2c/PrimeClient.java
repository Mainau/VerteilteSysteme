...
public class PrimeClient implements Runnable {
    ...
    public static synchronized long getWaitingTimeAVG(long currentWaitingTime){
        waitingTimeCount++;
        return (waitingTimeAVG+=currentWaitingTime)/(waitingTimeCount*1000);
    }

    public static synchronized long getProcessingTimeAVG(long currentProcessingTime){
        processingTimeCount++;
        return (processingTimeAVG+=currentProcessingTime)/(processingTimeCount*1000);
    }

    public static synchronized long getCompleteTimeAVG(long currentCompleteTime){
        completeTimeCount++;
        return (completeTimeAVG+=currentCompleteTime)/(completeTimeCount*1000);
    }
    ...
    public void processNumber(long value) throws IOException, ClassNotFoundException {
        ...
        synchronized (valueLock) {
            ...
            System.out.println(" | p: "+
                    processingTime/1000 +" ("+getProcessingTimeAVG(processingTime)+") ms |  w: "+
                    waitingTime/1000 +" ("+getWaitingTimeAVG(waitingTime)+") ms |  c: "+
                    completeTime /1000+" ("+getCompleteTimeAVG(completeTime)+") ms | ");
        }
    }
